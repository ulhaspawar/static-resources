  ...
  resourceLoadPromise = (async () => {
    try {
      ...
      await Promise.all([
        ...
        loadCSS(
          `https://cdn.jsdelivr.net/gh/ulhaspawar/static-resources@e17e0ec/eyllek/orbo-skin-analyzer.CR-DisvS.css?t=${timestamp}`
        ),
        loadScript(
          `https://makeup.sdk.orbo.ai/WAjLANasI8/4.0.0/smart-capture/orbo-clinikally-skinanalysis.js?t=${timestamp}`
        ),
        loadScript(
          `https://cdn.jsdelivr.net/gh/ulhaspawar/static-resources@e17e0ec/eyllek/orbo-skin-analyzer.umd.BNwvwJ_w.js?t=${timestamp}`
        ),
      ]);
...
function initializeSkinAnalyzer() {
  if (document.getElementById("orbo-cc-skin-analyzer-container")) {
    console.log("Skin analyzer already initialized");
    return;
  }
  ...
  function handleDOMContentLoaded() {
    ...
    if (!ca || ca !== "t") {
      ...
      if (
        typeof OrboSkinAnalyzerLib !== "undefined" &&
        OrboSkinAnalyzerLib.startOrboSkinAnalyzer
      ) {
        OrboSkinAnalyzerLib.startOrboSkinAnalyzer();
      } else {
